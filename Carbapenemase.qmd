...
CombineData<-
  full_join(MPCRFinal,CARBA5Final,by = join_by(ID, SPECIES)) %>%
  full_join(.,CIMFinal,by = join_by(ID, SPECIES)) %>%
  full_join(.,CPOFinal,by = join_by(ID, SPECIES))
...
KACOP<-
  ...
  scale_fill_gradient(low="#F7FBFF",high="#08306B",breaks=c(0,3,6,9,12))
ECCPO<-
...
CFCCPO<-
  ...
  geom_tile()+geom_text()+
  scale_x_discrete(guide = guide_axis(n.dodge=2))+
  scale_fill_gradient(low="#F7FBFF",high="#08306B",breaks=c(0,2,4,6,8))+
...
CombineData %>% filter(!is.na(CPO) &!is.na(MPCR)) #492
CPOMPCRGrpup<-
  CPO_MPCR %>% group_by(MPCR,MPCRGroup,CPO) %>%
  summarise(Count=n()) 
...
```{r warning=FALSE, message=FALSE,, fig.height=10,fig.width=15}
#| echo: false
CPO_MPCR %>% group_by(Name,MPCRGroup,CPO) %>%
  summarise(Total=n()) %>% 
  ggplot(aes(x=CPO,y=MPCRGroup,fill=Total,label=Total))+
...
```{r warning=F,message=F}
#| echo: false
CPO_MPCR %>% filter(Name=="Klebsiella pneumoniae")%>%
  group_by(MPCR,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=MPCR,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_MPCR %>% filter(Name=="Escherichia coli")%>%
  group_by(MPCR,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=MPCR,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_MPCR %>% filter(Name=="Enterobacter cloacae complex")%>%
  group_by(MPCR,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=MPCR,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_MPCR %>% filter(Name=="Citrobacter freundii complex")%>%
  group_by(MPCR,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=MPCR,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_MPCR %>% filter(Name=="Citrobacter koseri")%>%
  group_by(MPCR,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=MPCR,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_MPCR %>% filter(Name=="Klebsiella aerogenes")%>%
  group_by(MPCR,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=MPCR,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_MPCR %>% filter(Name=="Klebsiella oxytoca")%>%
  group_by(MPCR,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=MPCR,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
# why 434?
CPO_CARBA5 %>% group_by(CARBA5,CARBA5Group,CPO) %>%
  summarise(Count=n())%>%
...
```{r warning=F,message=F}
#| echo: false
CPO_CARBA5 %>% group_by(CARBA5Group,CPO) %>%
  summarise(Total=n()) %>% 
...
```{r warning=F,message=F}
#| echo: false
CPO_CARBA5 %>% filter(Name=="Klebsiella pneumoniae")%>%
  group_by(CARBA5,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=CARBA5,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_CARBA5 %>% filter(Name=="Escherichia coli")%>%
  group_by(CARBA5,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=CARBA5,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_CARBA5 %>% filter(Name=="Enterobacter cloacae complex")%>%
  group_by(CARBA5,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=CARBA5,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_CARBA5 %>% filter(Name=="Citrobacter freundii complex")%>%
  group_by(CARBA5,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=CARBA5,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_CARBA5 %>% filter(Name=="Citrobacter koseri")%>%
  group_by(CARBA5,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=CARBA5,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_CARBA5 %>% filter(Name=="Klebsiella aerogenes")%>%
  group_by(CARBA5,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=CARBA5,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
```{r warning=F,message=F}
#| echo: false
CPO_CARBA5 %>% filter(Name=="Klebsiella oxytoca")%>%
  group_by(CARBA5,CPO) %>%
  summarise(Count=n()) %>%
  ggplot(aes(x=CPO,y=CARBA5,fill=Count,label=Count))+
  geom_tile()+geom_text()+
...
#| echo: false
MICCPOCIM<-
  inner_join(MICCPO,CPO_CIM, by = c("ID", "Name", "SPECIES", "CPO"))
...
#| echo: false
MICCPOMPCR<-
  inner_join(MICCPO,MPCRFinal, by = c("ID", "SPECIES"))
...
#| echo: false
MICCPOCARBA5<-
  inner_join(MICCPO,CARBA5Final, by = c("ID", "SPECIES"))
...
CARBA5_MPCR %>% filter(!is.na(CARBA5) &!is.na(MPCR)) #499
CARBA5_MPCR$CARBA5Group<-fct_rev(CARBA5_MPCR$CARBA5Group)
CARBA5_MPCRGroup<-
  CARBA5_MPCR %>%
...
# PCR positive- CPO vs. CARBA5 {#pcr-positive--cpo-vs.-carba5}
